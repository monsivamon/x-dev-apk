...
# Constants.ktからサポート対象のバージョン文字列を抽出する
def get_supported_versions(constants: str) -> frozenset[str]:
    # デバッグ出力（確認用）
    print("===== Constants.kt =====")
    print(constants)
    print("===== Regex matches =====")
    matches = re.findall(r'version\s*=\s*"([^"]+)"', constants)
    print(matches)

    versions = frozenset(matches)
    if not versions:
        raise ValueError("Could not find dev compatible app versions in Piko")
    return versions

# Pikoをクローンしてパッチをビルドし、成果物を出力先にコピーする
def build_piko_patches(output: str = "bins/patches.mpp") -> PikoBuild:
    ...
    with tempfile.TemporaryDirectory(prefix="piko-") as temporary_directory:
        ...
        supported_versions = get_supported_versions(
            (piko_directory / XDEV_CONSTANTS).read_text()
        )
